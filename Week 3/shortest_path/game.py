...
class grid:
	def __init__(self, width, height):
		...
		self.possible_directions = [self.left, self.right, self.up, self.down]
	...
	def find_neighbors(self, node):
		...
		for edges in self.possible_directions:
			possible_neighbors.append(node + edges)
...
class player:
	...
	def move_up(self, screen, map):
		...
		if 0 <= new_pos.y < map.height:
			if new_pos.y not in map.obstacles:
				self.pos = new_pos
				self.draw(screen)
	...
	def move_left(self, screen, map):
		...
		if 0 <= new_pos.x < map.width:
			if new_pos.x not in map.obstacles:
				self.pos = new_pos
				self.draw(screen)
...
class game(grid, player):
	...
	def play(self):
		#infinite loop to keep the game going until the user quits the game
		self.map.add_rand_obstacles(10)
		while self.game_loop:
			pygame.display.set_caption("Sanjidah Wahid")
